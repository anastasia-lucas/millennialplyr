...
devtools::install_github("anastasia-lucas/millennialplyr")
```
...
| `left_join()`  | `for_real()`             | Left join to see which of your rows are legit? |
| `right_join()` | `adulting()`             | A redundant function to make your code sound harder than it actually is. |
| `inner_join()` | `tight()`                | Only keep the rows that are tight with each other. |
